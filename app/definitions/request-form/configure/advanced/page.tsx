...
export default function RequestFormAdvancedPage() {
  ...
  const getFieldsByColumn = (column: 1 | 2) => {
    return fields
      .filter(f => f.category === 'details' && f.column === column)
  ...
  const handleDrop = (targetFieldId: string, targetColumn: 1 | 2) => {
    ...
    if (!draggingField || !targetField || draggingField.category !== 'details' || targetField.category !== 'details') return;

    setFields(prevFields => {
      const newFields = [...prevFields];
      const categoryFields = newFields.filter(f => f.category === 'details' && f.column === targetColumn).sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
  ...
  const handleDropOnColumn = (column: 1 | 2) => {
    ...
    const draggingField = fields.find(f => f.id === draggingFieldId);
    if (!draggingField || draggingField.category !== 'details') return;
  ...
  const handleAddField = (fieldData: { inputType: FieldInputType; label: string; dropdownOptions?: string[]; }) => {
    const newField: RequestFormField = {
      ...
      systemFixed: false,
      order: fields.filter(f => f.category === 'details').length,
      column: 1,
      placeholder: '',
            ...
            <div className="bg-gradient-to-br from-primary/5 to-primary/10 border border-primary/20 rounded-xl p-5 sticky top-20">
              ...
              <div className="mb-4">
                ...
                <p className="text-xs text-muted-foreground">
                  Advanced fields capture supplementary request details like loan programs, property conditions, and special instructions. These fields support complex workflows and provide flexibility for different request types.
                </p>
              </div>
                ...
                <div>
                  ...
                  <div className="relative w-full aspect-video bg-gradient-to-br from-slate-200 via-slate-300 to-slate-400 rounded-lg overflow-hidden">
                    <div className="absolute inset-0 flex items-center justify-center">
                      <button className="w-14 h-14 bg-white/90 hover:bg-white rounded-full flex items-center justify-center transition-all hover:scale-110 shadow-lg" aria-label="Play video: Advanced Request Fields">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-3">
                      <p className="text-white text-xs font-medium">Advanced Request Fields</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <ul className="space-y-1.5 text-xs text-muted-foreground">
                    <li className="flex items-start gap-2">
                      ...
                      Loan program and purpose details
                    </li>
                    <li className="flex items-start gap-2">
                      <svg className="w-3 h-3 text-green-600 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-xs text-blue-900">
                      ...
                      <p>Mark advanced fields as optional if they only apply to specific request types. This keeps forms flexible without overwhelming users.</p>
                    </div>
                  </div>
                </div>
              </div>
