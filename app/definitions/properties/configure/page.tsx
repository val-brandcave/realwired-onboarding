...
export default function PropertyRecordConfigurePage() {
  ...
  const [fields, setFields] = useState<PropertyRecordField[]>(() => {
    return state.definitions.propertyRecordFields.map((field, index) => ({
      ...field,
  ...
  const getFieldsBySection = (category: 'overview' | 'advanced', column: 1 | 2) => {
    return fields
  ...
  const handleDrop = (targetFieldId: string, targetColumn: 1 | 2, targetCategory: 'overview' | 'advanced') => {
  ...
  const handleDropOnColumn = (column: 1 | 2, category: 'overview' | 'advanced') => {
  ...
  const handleOpenAddFieldModal = (category: 'overview' | 'advanced') => {
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-card border border-border rounded-xl overflow-hidden">
              ...
              <div className="p-6 border-b border-border flex items-center justify-between">
                ...
                <button
                  onClick={() => handleOpenAddFieldModal('advanced')}
                  className="px-4 py-2 text-sm font-medium text-primary-foreground bg-primary hover:bg-primary/90 rounded-lg transition-colors flex items-center gap-2"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div
                  className="p-6 min-h-[400px]"
                  onDragOver={handleDragOver}
                  onDrop={() => handleDropOnColumn(1, 'advanced')}
                >
                  {getFieldsBySection('advanced', 1).map((field) => (
                    <DraggableField
                      key={field.id}
                      field={field}
                      isSelected={selectedFieldId === field.id}
                      ...
                      onDragEnd={handleDragEnd}
                      onDragOver={handleDragOver}
                      onDrop={(targetId) => handleDrop(targetId, 1, 'advanced')}
                      isDragging={draggingFieldId === field.id}
                    />
                  ))}
                  {getFieldsBySection('advanced', 1).length === 0 && (
                    <div className="flex items-center justify-center h-32 border-2 border-dashed border-border rounded-lg">
                      <p className="text-sm text-muted-foreground">Drag fields here</p>
                    </div>
                  )}
                ...
                <div
                  className="p-6 min-h-[400px]"
                  onDragOver={handleDragOver}
                  onDrop={() => handleDropOnColumn(2, 'advanced')}
                >
                  {getFieldsBySection('advanced', 2).map((field) => (
                    <DraggableField
                      key={field.id}
                      field={field}
                      isSelected={selectedFieldId === field.id}
                      ...
                      onDragEnd={handleDragEnd}
                      onDragOver={handleDragOver}
                      onDrop={(targetId) => handleDrop(targetId, 2, 'advanced')}
                      isDragging={draggingFieldId === field.id}
                    />
                  ))}
                  {getFieldsBySection('advanced', 2).length === 0 && (
                    <div className="flex items-center justify-center h-32 border-2 border-dashed border-border rounded-lg">
                      <p className="text-sm text-muted-foreground">Drag fields here</p>
                    </div>
                  )}
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-xs text-blue-900">
                      <strong>Tip:</strong> Click any field to configure it. Drag fields to reorder them within columns or move between columns.
                    </p>
                  </div>
                </div>
              </div>
